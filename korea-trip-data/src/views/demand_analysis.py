...
def render_demand_analysis():
    ...
    if df_ota.empty:
        ...
        with col2:
            ...
            st.plotly_chart(fig3, use_container_width=True, key='chart_demand_analysis_fig3_6')
        ...
        fig_infra.update_layout(
            ...
            hoverlabel=dict(bgcolor="#E2E8F0", font_size=13, font_family="Pretendard", font=dict(color="#F8FAFC")),
            margin=dict(l=20, r=20, t=40, b=20),
            xaxis=dict(showgrid=True, gridcolor="#E2E8F0", zeroline=False, linecolor="#CBD5E1", title="인프라 개수"),
            yaxis_title=None,
            legend_title_text='인프라 종류'
        )
        st.plotly_chart(fig_infra, use_container_width=True, key='chart_demand_analysis_fig_infra_9')
...
3. **'소수 정예' 지역 축제 육성**: 체험, 자연, 전통 등의 테마를 결합하여 외부 관광객을 확실히 유인할 수 있는 밀도 높고 독창적인 지역 축제 발굴에 선택과 집중을 해야 합니다.
        ...
        if len(merged) > 0:
            ...
            fig_scatter.add_annotation(
                ...
                bgcolor="rgba(0, 0, 0, 0.03)", bordercolor="rgba(0, 0, 0, 0.1)",
